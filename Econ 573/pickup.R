...
setwd('C:/Users/babiy/OneDrive/Documents/R')
...
## explore a bit
head(trucks) # first rows
dim(trucks) # sample size and number of variables
...
class(trucks$make) # it is a 'character' variable
trucks$make <- as.factor(trucks$make) # convert to a 'factor'
...
boxplot(price ~ make, data=trucks) ## a boxplot
plot(price~miles, data=trucks) ## simple scatterplot
plot(price~miles, data=trucks, log="y", col=factor(trucks$make)) ## in color
...
fit <- glm(log(price) ~ make + miles + year, data=trucks) # ~. means `on everything'
